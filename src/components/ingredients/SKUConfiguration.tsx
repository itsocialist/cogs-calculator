        ...
        >
            ...
            <div className="mb-4 p-3 bg-white/5 rounded-lg border border-white/10">
                <div className="flex justify-between text-xs text-white/50 mb-1">
                    ...
                    <span className={isOverAllocated ? 'text-red-400 font-bold' : 'text-white/70'}>
                ...
                {isOverAllocated && (
                    <p className="text-xs text-red-400 mt-1">⚠️ Over-allocated by
            ...
            <div className="space-y-3">
                {skus.map((sku) => {
                    ...
                    return (
                        <div key={sku.id} className="border border-white/15 rounded-lg overflow-hidden bg-white/5">
                            {/* SKU Header */}
                            <div className="p-3">
                                <div className="flex items-center gap-3">
                                    ...
                                    <div className="flex-1 grid grid-cols-2 md:grid-cols-6 gap-3">
                                        <input
                                            ...
                                            className="bg-transparent font-medium text-white/90 focus:outline-none focus:border-b focus:border-amber-400/50"
                                        ...
                                        <div className="text-right">
                                            <div className="text-xs text-white/40">WS Margin</div>
                                            <div className={`font-mono font-bold text-sm ${calc && calc.wholesaleMargin < 0 ? 'text-red-400' : 'text-green-400'}`}>
                                ...
                                {calc && (
                                    <div className="mt-2 grid grid-cols-2 md:grid-cols-4 lg:grid-cols-6 gap-2 text-xs">
                                        <div className="bg-white/5 border border-white/10 rounded px-2 py-1">
                                            <div className="text-white/40">Potency</div>
                                            <div className={calc.isPotencySafe ? 'font-bold text-green-400' : 'font-bold text-red-400'}>{calc.potencyMg.toFixed(0)}mg</div>
                                        </div>
                                        <div className="bg-white/5 border border-white/10 rounded px-2 py-1">
                                            <div className="text-white/40">Formula</div>
                                            <div className="font-mono text-white/80">${calc.formulaCostPerUnit.toFixed(2)}</div>
                                        </div>
                                        <div className="bg-white/5 border border-white/10 rounded px-2 py-1">
                                            <div className="text-white/40">Packaging</div>
                                            <div className="font-mono text-white/80">${calc.packagingCostPerUnit.toFixed(2)}</div>
                                        </div>
                                        <div className="bg-white/5 border border-white/10 rounded px-2 py-1">
                                            <div className="text-white/40">Labor+Fill</div>
                                            <div className="font-mono text-white/80">${(calc.laborCostPerUnit + calc.fulfillmentCostPerUnit).toFixed(2)}</div>
                                        </div>
                                        <div className="bg-white/5 border border-white/10 rounded px-2 py-1">
                                            <div className="text-white/40">COGS</div>
                                            <div className="font-mono font-bold text-white/90">${calc.fullyLoadedCost.toFixed(2)}</div>
                                        </div>
                                        <div className={`rounded px-2 py-1 border ${calc.wholesaleMargin >= 0 ? 'bg-green-500/10 border-green-500/30' : 'bg-red-500/10 border-red-500/30'}`}>
                                            <div className="text-white/40">Margin</div>
                                            <div className={`font-mono font-bold ${calc.wholesaleMargin >= 0 ? 'text-green-400' : 'text-red-400'}`}>
                                ...
                                isExpanded && (
                                    <div className="border-t border-white/10 bg-white/5 p-3">
                                        <div className="flex justify-between items-center mb-2">
                                            <h4 className="text-xs font-bold text-white/50 uppercase">Packaging for {sku.name}</h4>
                                            <button
                                                onClick={() => onAddPackagingItem(sku.id, { name: "New Item", costPerUnit: 0 })}
                                                className="text-xs text-amber-300/80 hover:text-amber-300 transition-colors"
                                            >
                                                + Add Item
                                            </button>
                                        </div>
                                        <div className="space-y-2">
                                            {sku.packaging.map((pkg) => (
                                                <div key={pkg.id} className="flex items-center gap-2 bg-white/5 p-2 rounded border border-white/10">
                                                    <input
                                                        type="text"
                                                        value={pkg.name}
                                                        onChange={(e) => {
                                                        ...
                                                        className="flex-1 text-sm px-2 py-1 bg-white/10 border border-white/20 rounded text-white/90 placeholder-white/40 focus:outline-none focus:border-amber-400/50"
                ...
                {/* Add New SKU Form */}
                {isAdding && (
                    <div className="border border-amber-500/30 bg-amber-500/10 rounded-lg p-3 animate-in fade-in">
                        <div className="grid grid-cols-2 md:grid-cols-6 gap-3">
                            <input
                                autoFocus
                                placeholder="SKU Name (e.g., 2oz Tin)"
                                className="bg-white/10 border border-white/20 rounded px-2 py-1 text-sm text-white placeholder-white/40 focus:outline-none focus:border-amber-400/50"
                                value={newSku.name}
                            ...
                            <div className="flex gap-1 items-end">
                                ...
                                <select
                                    ...
                                    className="bg-white/15 backdrop-blur-sm border-0 rounded px-2 py-2 text-xs font-semibold text-white focus:outline-none focus:ring-1 focus:ring-white/40 hover:bg-white/20 transition-all"
                                >
                                    <option value="g">g</option>
                                    <option value="ml">ml</option>
                                    <option value="oz">oz</option>
                            ...
                            <button
                                onClick={handleAdd}
                                className="bg-white/20 text-white rounded px-4 py-2 text-sm font-medium hover:bg-white/30 transition-colors"
                            >
                                Add SKU
                            </button>
                        </div>
                        <p className="text-xs text-white/40 mt-2">Default packaging will be copied. Expand to customize.</p>
                    </div>
                )}
            </div>
        </Card >
